...
export function RetireConfirmModal({ hp, onConfirm, onCancel }: { hp: number; onConfirm: () => void; onCancel: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="mx-3 mb-3 p-4 rounded-[var(--radius-md)] flex flex-col gap-3"
      style={{ background: 'var(--color-surface-strong)', boxShadow: 'var(--shadow-elevated)' }}
    >
      <p className="text-sm font-medium text-center whitespace-pre-line" style={{ color: 'var(--color-text-primary)' }}>
        {msg.title}
      </p>
      <div className="flex flex-col gap-2">
        <button
          type="button"
          onClick={onConfirm}
          className="w-full py-2.5 rounded-[var(--radius-full)] text-sm font-bold transition-all active:scale-[0.98] focus:outline-none"
        ...
        <button
          type="button"
          onClick={onCancel}
          className="w-full py-2.5 rounded-[var(--radius-full)] text-sm font-medium transition-all active:scale-[0.98] focus:outline-none"
...
export function TimelinePanel() {
  ...
  useEffect(() => {
    if (eventListRef.current) setLineHeight(eventListRef.current.offsetHeight);
  }, [eventLog]);
      ...
      >
        {!hasContent ? (
          <div className="flex items-center justify-center h-full px-4 py-8">
            <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>아직 기록이 없어요 👀</p>
          </div>
        ) : (
          <div className="relative" style={{ padding: '8px 8px 8px 0' }}>
            {eventLog.length > 0 && (
              <div className="pointer-events-none" style={{ position: 'absolute', top: 0, height: lineHeight, left: 52, width: 2, background: 'var(--color-timeline-line)' }} aria-hidden />
            )}
              ...
              <AnimatePresence initial={false}>
                {eventLog.map((log) => (
                  ...
                  >
                    ...
                    {!isReadOnly && (
                      <button type="button" onClick={() => removeEvent(log.id)} className="shrink-0 w-5 h-5 ml-1 flex items-center justify-center rounded-full opacity-30 hover:opacity-100 transition-opacity duration-150 focus:outline-none" style={{ background: 'rgba(0,0,0,0.10)', color: 'var(--color-text-muted)', cursor: 'pointer' }} aria-label={`${log.name} 삭제`}>
            ...
            {reactionEntries.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} style={{ marginTop: eventLog.length > 0 ? 8 : 0, paddingTop: eventLog.length > 0 ? 12 : 4, paddingLeft: 8, borderTop: eventLog.length > 0 ? '1px solid var(--color-border)' : 'none' }}>
                <p className="text-xs font-medium mb-2" style={{ color: 'var(--color-text-muted)', paddingLeft: 4 }}>팀원 응원 도착 🎉</p>
                <div className="flex flex-wrap gap-2 px-1">
                  {reactionEntries.map(({ emoji, count }) => (
                    <div key={emoji} className="flex items-center gap-1 px-3 py-1.5 rounded-full text-sm" style={{ background: 'rgba(0,0,0,0.06)', color: 'var(--color-text-secondary)' }}>
                      <span>{emoji}</span>
